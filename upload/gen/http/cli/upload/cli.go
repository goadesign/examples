...
func UsageExamples() string {
	return os.Args[0] + ` upload head --id "666dfedc1646ac7a507aa76ebaa109b8" --tus-resumable "1.0.0" --upload-offset 12314928381822528796` + "\n" +
		""
}
...
func ParseEndpoint(
	...
	var (
		...
		uploadPostUploadLengthFlag      = uploadPostFlags.String("upload-length", "", "")
		...
		uploadPostUploadMetadataFlag    = uploadPostFlags.String("upload-metadata", "", "")
	...
	)
	{
		...
		case "upload":
			...
			case "post":
				endpoint = c.Post()
				data, err = uploadc.BuildPostPayload(*uploadPostBodyFlag, *uploadPostTusResumableFlag, *uploadPostUploadLengthFlag, *uploadPostUploadDeferLengthFlag, *uploadPostUploadChecksumFlag, *uploadPostUploadMetadataFlag, *uploadPostTusMaxSizeFlag)
			case "delete":
				endpoint = c.Delete()
...
Example:
    `+os.Args[0]+` upload head --id "666dfedc1646ac7a507aa76ebaa109b8" --tus-resumable "1.0.0" --upload-offset 12314928381822528796
`, os.Args[0])
}
...
Example:
    `+os.Args[0]+` upload patch --body "QWNjdXNhbnRpdW0gdXQgdm9sdXB0YXMgZXQgb21uaXMgYW5pbWku" --id "844e3408e54c83d5b8e764c0c7ca3d11" --tus-resumable "1.0.0" --upload-offset 17279240799857888469 --upload-checksum "sha1 Kq5sNclPz7QV2+lfQIuc6R7oRu0="
`, os.Args[0])
}
...
func uploadPostUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] upload post -body STRING -tus-resumable STRING -upload-length UINT -upload-defer-length INT -upload-checksum STRING -upload-metadata STRING -tus-max-size UINT
...
Example:
    `+os.Args[0]+` upload post --body "Q29ycG9yaXMgZWEgZXQgY29ycG9yaXMgdm9sdXB0YXRlbS4=" --tus-resumable "1.0.0" --upload-length 9880729877547457329 --upload-defer-length 1 --upload-checksum "sha1 Kq5sNclPz7QV2+lfQIuc6R7oRu0=" --upload-metadata "key1 val1,key2 val2" --tus-max-size 12868988164786391894
`, os.Args[0])
}
