...
import (
	...
	securedservicec "goa.design/examples/security/multiauth/gen/http/secured_service/client"
...
func securedServiceSigninUsage() {
	...
	// Flags list
	fmt.Fprintln(os.Stderr, `    -username STRING: Username used to perform signin`)
	fmt.Fprintln(os.Stderr, `    -password STRING: Password used to perform signin`)
...
func securedServiceSecureUsage() {
	...
	fmt.Fprint(os.Stderr, " -fail BOOL")
	...
	// Flags list
	fmt.Fprintln(os.Stderr, `    -fail BOOL: `)
	fmt.Fprintln(os.Stderr, `    -token STRING: `)

	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, "Example:")
	fmt.Fprintf(os.Stderr, "    %s %s\n", os.Args[0], "secured-service secure --fail false --token \"Impedit impedit sed eos aut.\"")
}
...
func securedServiceAlsoDoublySecureUsage() {
	...
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, "Example:")
	fmt.Fprintf(os.Stderr, "    %s %s\n", os.Args[0], "secured-service also-doubly-secure --key \"abcdef12345\" --oauth-token \"Dicta explicabo dolore est.\" --token \"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWV9.TJVA95OrM7E2cBab30RMHrHDcEfxjoYZgeFONFh7HgQ\" --username \"user\" --password \"password\"")
}
