...
func NewMultiUpdateRequestBody(p *storage.MultiUpdatePayload) *MultiUpdateRequestBody {
	...
	if p.Bottles != nil {
		...
		body.Bottles = []*BottleRequestBody{}
	}
	return body
}
...
func NewListStoredBottleCollectionOK(body StoredBottleResponseTinyCollection) storageviews.StoredBottleCollectionView {
	...
	for i, val := range body {
		...
		v[i] = unmarshalStoredBottleResponseTinyToStorageviewsStoredBottleView(val)
	}
...
func ValidateShowResponseBody(body *ShowResponseBody) (err error) {
	...
	if body.Winery != nil {
		if err2 := validateWineryResponseBody(body.Winery, "body.winery"); err2 != nil {
			err = goa.MergeErrors(err, err2)
		}
	}
	if body.Vintage != nil {
...
// validateWineryResponseBody checks Winery and reports errors using the path
// supplied by its caller
func validateWineryResponseBody(body *WineryResponseBody, path string) (err error) {
	if body.Name == nil {
		err = goa.MergeErrors(err, goa.MissingFieldError("name", path))
	}
	if body.Region == nil {
