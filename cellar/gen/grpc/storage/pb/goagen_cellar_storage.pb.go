// Code generated with goa v3.23.0, DO NOT EDIT.
//
// storage protocol buffer definition
//
// Command:
// $ goa gen goa.design/examples/cellar/design

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        v6.33.0
// source: goagen_cellar_storage.proto

package storagepb

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ListRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRequest) Reset() {
	*x = ListRequest{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRequest) ProtoMessage() {}

func (x *ListRequest) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRequest.ProtoReflect.Descriptor instead.
func (*ListRequest) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{0}
}

type StoredBottleCollection struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Field         []*StoredBottle        `protobuf:"bytes,1,rep,name=field,proto3" json:"field,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StoredBottleCollection) Reset() {
	*x = StoredBottleCollection{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StoredBottleCollection) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StoredBottleCollection) ProtoMessage() {}

func (x *StoredBottleCollection) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StoredBottleCollection.ProtoReflect.Descriptor instead.
func (*StoredBottleCollection) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{1}
}

func (x *StoredBottleCollection) GetField() []*StoredBottle {
	if x != nil {
		return x.Field
	}
	return nil
}

// A StoredBottle describes a bottle retrieved by the storage service.
type StoredBottle struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID is the unique id of the bottle.
	Id string `protobuf:"bytes,8,opt,name=id,proto3" json:"id,omitempty"`
	// Name of bottle
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// Winery that produces wine
	Winery *Winery `protobuf:"bytes,3,opt,name=winery,proto3" json:"winery,omitempty"`
	// Vintage of bottle
	Vintage uint32 `protobuf:"varint,4,opt,name=vintage,proto3" json:"vintage,omitempty"`
	// Composition is the list of grape varietals and associated percentage.
	Composition []*Component `protobuf:"bytes,5,rep,name=composition,proto3" json:"composition,omitempty"`
	// Description of bottle
	Description *string `protobuf:"bytes,6,opt,name=description,proto3,oneof" json:"description,omitempty"`
	// Rating of bottle from 1 (worst) to 5 (best)
	Rating        *uint32 `protobuf:"varint,7,opt,name=rating,proto3,oneof" json:"rating,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StoredBottle) Reset() {
	*x = StoredBottle{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StoredBottle) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StoredBottle) ProtoMessage() {}

func (x *StoredBottle) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StoredBottle.ProtoReflect.Descriptor instead.
func (*StoredBottle) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{2}
}

func (x *StoredBottle) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *StoredBottle) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *StoredBottle) GetWinery() *Winery {
	if x != nil {
		return x.Winery
	}
	return nil
}

func (x *StoredBottle) GetVintage() uint32 {
	if x != nil {
		return x.Vintage
	}
	return 0
}

func (x *StoredBottle) GetComposition() []*Component {
	if x != nil {
		return x.Composition
	}
	return nil
}

func (x *StoredBottle) GetDescription() string {
	if x != nil && x.Description != nil {
		return *x.Description
	}
	return ""
}

func (x *StoredBottle) GetRating() uint32 {
	if x != nil && x.Rating != nil {
		return *x.Rating
	}
	return 0
}

type Winery struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of winery
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Region of winery
	Region string `protobuf:"bytes,2,opt,name=region,proto3" json:"region,omitempty"`
	// Country of winery
	Country string `protobuf:"bytes,3,opt,name=country,proto3" json:"country,omitempty"`
	// Winery website URL
	Url           *string `protobuf:"bytes,4,opt,name=url,proto3,oneof" json:"url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Winery) Reset() {
	*x = Winery{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Winery) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Winery) ProtoMessage() {}

func (x *Winery) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Winery.ProtoReflect.Descriptor instead.
func (*Winery) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{3}
}

func (x *Winery) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Winery) GetRegion() string {
	if x != nil {
		return x.Region
	}
	return ""
}

func (x *Winery) GetCountry() string {
	if x != nil {
		return x.Country
	}
	return ""
}

func (x *Winery) GetUrl() string {
	if x != nil && x.Url != nil {
		return *x.Url
	}
	return ""
}

type Component struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Grape varietal
	Varietal string `protobuf:"bytes,1,opt,name=varietal,proto3" json:"varietal,omitempty"`
	// Percentage of varietal in wine
	Percentage    *uint32 `protobuf:"varint,2,opt,name=percentage,proto3,oneof" json:"percentage,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Component) Reset() {
	*x = Component{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Component) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Component) ProtoMessage() {}

func (x *Component) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Component.ProtoReflect.Descriptor instead.
func (*Component) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{4}
}

func (x *Component) GetVarietal() string {
	if x != nil {
		return x.Varietal
	}
	return ""
}

func (x *Component) GetPercentage() uint32 {
	if x != nil && x.Percentage != nil {
		return *x.Percentage
	}
	return 0
}

type ShowNotFoundError struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Message of error
	Message_ string `protobuf:"bytes,1,opt,name=message_,json=message,proto3" json:"message_,omitempty"`
	// ID of missing bottle
	Id            string `protobuf:"bytes,2,opt,name=id,proto3" json:"id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ShowNotFoundError) Reset() {
	*x = ShowNotFoundError{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ShowNotFoundError) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShowNotFoundError) ProtoMessage() {}

func (x *ShowNotFoundError) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShowNotFoundError.ProtoReflect.Descriptor instead.
func (*ShowNotFoundError) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{5}
}

func (x *ShowNotFoundError) GetMessage_() string {
	if x != nil {
		return x.Message_
	}
	return ""
}

func (x *ShowNotFoundError) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

type ShowRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID of bottle to show
	Id            string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ShowRequest) Reset() {
	*x = ShowRequest{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ShowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShowRequest) ProtoMessage() {}

func (x *ShowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShowRequest.ProtoReflect.Descriptor instead.
func (*ShowRequest) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{6}
}

func (x *ShowRequest) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

type ShowResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID is the unique id of the bottle.
	Id string `protobuf:"bytes,8,opt,name=id,proto3" json:"id,omitempty"`
	// Name of bottle
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// Winery that produces wine
	Winery *Winery `protobuf:"bytes,3,opt,name=winery,proto3" json:"winery,omitempty"`
	// Vintage of bottle
	Vintage uint32 `protobuf:"varint,4,opt,name=vintage,proto3" json:"vintage,omitempty"`
	// Composition is the list of grape varietals and associated percentage.
	Composition []*Component `protobuf:"bytes,5,rep,name=composition,proto3" json:"composition,omitempty"`
	// Description of bottle
	Description *string `protobuf:"bytes,6,opt,name=description,proto3,oneof" json:"description,omitempty"`
	// Rating of bottle from 1 (worst) to 5 (best)
	Rating        *uint32 `protobuf:"varint,7,opt,name=rating,proto3,oneof" json:"rating,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ShowResponse) Reset() {
	*x = ShowResponse{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ShowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShowResponse) ProtoMessage() {}

func (x *ShowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShowResponse.ProtoReflect.Descriptor instead.
func (*ShowResponse) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{7}
}

func (x *ShowResponse) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *ShowResponse) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ShowResponse) GetWinery() *Winery {
	if x != nil {
		return x.Winery
	}
	return nil
}

func (x *ShowResponse) GetVintage() uint32 {
	if x != nil {
		return x.Vintage
	}
	return 0
}

func (x *ShowResponse) GetComposition() []*Component {
	if x != nil {
		return x.Composition
	}
	return nil
}

func (x *ShowResponse) GetDescription() string {
	if x != nil && x.Description != nil {
		return *x.Description
	}
	return ""
}

func (x *ShowResponse) GetRating() uint32 {
	if x != nil && x.Rating != nil {
		return *x.Rating
	}
	return 0
}

type AddRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of bottle
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Winery that produces wine
	Winery *Winery `protobuf:"bytes,2,opt,name=winery,proto3" json:"winery,omitempty"`
	// Vintage of bottle
	Vintage uint32 `protobuf:"varint,3,opt,name=vintage,proto3" json:"vintage,omitempty"`
	// Composition is the list of grape varietals and associated percentage.
	Composition []*Component `protobuf:"bytes,4,rep,name=composition,proto3" json:"composition,omitempty"`
	// Description of bottle
	Description *string `protobuf:"bytes,5,opt,name=description,proto3,oneof" json:"description,omitempty"`
	// Rating of bottle from 1 (worst) to 5 (best)
	Rating        *uint32 `protobuf:"varint,6,opt,name=rating,proto3,oneof" json:"rating,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AddRequest) Reset() {
	*x = AddRequest{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddRequest) ProtoMessage() {}

func (x *AddRequest) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddRequest.ProtoReflect.Descriptor instead.
func (*AddRequest) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{8}
}

func (x *AddRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *AddRequest) GetWinery() *Winery {
	if x != nil {
		return x.Winery
	}
	return nil
}

func (x *AddRequest) GetVintage() uint32 {
	if x != nil {
		return x.Vintage
	}
	return 0
}

func (x *AddRequest) GetComposition() []*Component {
	if x != nil {
		return x.Composition
	}
	return nil
}

func (x *AddRequest) GetDescription() string {
	if x != nil && x.Description != nil {
		return *x.Description
	}
	return ""
}

func (x *AddRequest) GetRating() uint32 {
	if x != nil && x.Rating != nil {
		return *x.Rating
	}
	return 0
}

type AddResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Field         string                 `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AddResponse) Reset() {
	*x = AddResponse{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddResponse) ProtoMessage() {}

func (x *AddResponse) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddResponse.ProtoReflect.Descriptor instead.
func (*AddResponse) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{9}
}

func (x *AddResponse) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

type RemoveRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID of bottle to remove
	Id            string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveRequest) Reset() {
	*x = RemoveRequest{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveRequest) ProtoMessage() {}

func (x *RemoveRequest) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveRequest.ProtoReflect.Descriptor instead.
func (*RemoveRequest) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{10}
}

func (x *RemoveRequest) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

type RemoveResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveResponse) Reset() {
	*x = RemoveResponse{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveResponse) ProtoMessage() {}

func (x *RemoveResponse) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveResponse.ProtoReflect.Descriptor instead.
func (*RemoveResponse) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{11}
}

type RateRequest struct {
	state         protoimpl.MessageState    `protogen:"open.v1"`
	Field         map[uint32]*ArrayOfString `protobuf:"bytes,1,rep,name=field,proto3" json:"field,omitempty" protobuf_key:"varint,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RateRequest) Reset() {
	*x = RateRequest{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RateRequest) ProtoMessage() {}

func (x *RateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RateRequest.ProtoReflect.Descriptor instead.
func (*RateRequest) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{12}
}

func (x *RateRequest) GetField() map[uint32]*ArrayOfString {
	if x != nil {
		return x.Field
	}
	return nil
}

type ArrayOfString struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Field         []string               `protobuf:"bytes,1,rep,name=field,proto3" json:"field,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ArrayOfString) Reset() {
	*x = ArrayOfString{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ArrayOfString) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ArrayOfString) ProtoMessage() {}

func (x *ArrayOfString) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ArrayOfString.ProtoReflect.Descriptor instead.
func (*ArrayOfString) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{13}
}

func (x *ArrayOfString) GetField() []string {
	if x != nil {
		return x.Field
	}
	return nil
}

type RateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RateResponse) Reset() {
	*x = RateResponse{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RateResponse) ProtoMessage() {}

func (x *RateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RateResponse.ProtoReflect.Descriptor instead.
func (*RateResponse) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{14}
}

type MultiAddRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Field         []*Bottle              `protobuf:"bytes,1,rep,name=field,proto3" json:"field,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MultiAddRequest) Reset() {
	*x = MultiAddRequest{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MultiAddRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MultiAddRequest) ProtoMessage() {}

func (x *MultiAddRequest) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MultiAddRequest.ProtoReflect.Descriptor instead.
func (*MultiAddRequest) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{15}
}

func (x *MultiAddRequest) GetField() []*Bottle {
	if x != nil {
		return x.Field
	}
	return nil
}

// Bottle describes a bottle of wine to be stored.
type Bottle struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of bottle
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Winery that produces wine
	Winery *Winery `protobuf:"bytes,2,opt,name=winery,proto3" json:"winery,omitempty"`
	// Vintage of bottle
	Vintage uint32 `protobuf:"varint,3,opt,name=vintage,proto3" json:"vintage,omitempty"`
	// Composition is the list of grape varietals and associated percentage.
	Composition []*Component `protobuf:"bytes,4,rep,name=composition,proto3" json:"composition,omitempty"`
	// Description of bottle
	Description *string `protobuf:"bytes,5,opt,name=description,proto3,oneof" json:"description,omitempty"`
	// Rating of bottle from 1 (worst) to 5 (best)
	Rating        *uint32 `protobuf:"varint,6,opt,name=rating,proto3,oneof" json:"rating,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Bottle) Reset() {
	*x = Bottle{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Bottle) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Bottle) ProtoMessage() {}

func (x *Bottle) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Bottle.ProtoReflect.Descriptor instead.
func (*Bottle) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{16}
}

func (x *Bottle) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Bottle) GetWinery() *Winery {
	if x != nil {
		return x.Winery
	}
	return nil
}

func (x *Bottle) GetVintage() uint32 {
	if x != nil {
		return x.Vintage
	}
	return 0
}

func (x *Bottle) GetComposition() []*Component {
	if x != nil {
		return x.Composition
	}
	return nil
}

func (x *Bottle) GetDescription() string {
	if x != nil && x.Description != nil {
		return *x.Description
	}
	return ""
}

func (x *Bottle) GetRating() uint32 {
	if x != nil && x.Rating != nil {
		return *x.Rating
	}
	return 0
}

type MultiAddResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Field         []string               `protobuf:"bytes,1,rep,name=field,proto3" json:"field,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MultiAddResponse) Reset() {
	*x = MultiAddResponse{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MultiAddResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MultiAddResponse) ProtoMessage() {}

func (x *MultiAddResponse) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MultiAddResponse.ProtoReflect.Descriptor instead.
func (*MultiAddResponse) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{17}
}

func (x *MultiAddResponse) GetField() []string {
	if x != nil {
		return x.Field
	}
	return nil
}

type MultiUpdateRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// IDs of the bottles to be updated
	Ids []string `protobuf:"bytes,1,rep,name=ids,proto3" json:"ids,omitempty"`
	// Array of bottle info that matches the ids attribute
	Bottles       []*Bottle `protobuf:"bytes,2,rep,name=bottles,proto3" json:"bottles,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MultiUpdateRequest) Reset() {
	*x = MultiUpdateRequest{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MultiUpdateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MultiUpdateRequest) ProtoMessage() {}

func (x *MultiUpdateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MultiUpdateRequest.ProtoReflect.Descriptor instead.
func (*MultiUpdateRequest) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{18}
}

func (x *MultiUpdateRequest) GetIds() []string {
	if x != nil {
		return x.Ids
	}
	return nil
}

func (x *MultiUpdateRequest) GetBottles() []*Bottle {
	if x != nil {
		return x.Bottles
	}
	return nil
}

type MultiUpdateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MultiUpdateResponse) Reset() {
	*x = MultiUpdateResponse{}
	mi := &file_goagen_cellar_storage_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MultiUpdateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MultiUpdateResponse) ProtoMessage() {}

func (x *MultiUpdateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_goagen_cellar_storage_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MultiUpdateResponse.ProtoReflect.Descriptor instead.
func (*MultiUpdateResponse) Descriptor() ([]byte, []int) {
	return file_goagen_cellar_storage_proto_rawDescGZIP(), []int{19}
}

var File_goagen_cellar_storage_proto protoreflect.FileDescriptor

const file_goagen_cellar_storage_proto_rawDesc = "" +
	"\n" +
	"\x1bgoagen_cellar_storage.proto\x12\astorage\"\r\n" +
	"\vListRequest\"E\n" +
	"\x16StoredBottleCollection\x12+\n" +
	"\x05field\x18\x01 \x03(\v2\x15.storage.StoredBottleR\x05field\"\x8a\x02\n" +
	"\fStoredBottle\x12\x0e\n" +
	"\x02id\x18\b \x01(\tR\x02id\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12'\n" +
	"\x06winery\x18\x03 \x01(\v2\x0f.storage.WineryR\x06winery\x12\x18\n" +
	"\avintage\x18\x04 \x01(\rR\avintage\x124\n" +
	"\vcomposition\x18\x05 \x03(\v2\x12.storage.ComponentR\vcomposition\x12%\n" +
	"\vdescription\x18\x06 \x01(\tH\x00R\vdescription\x88\x01\x01\x12\x1b\n" +
	"\x06rating\x18\a \x01(\rH\x01R\x06rating\x88\x01\x01B\x0e\n" +
	"\f_descriptionB\t\n" +
	"\a_rating\"m\n" +
	"\x06Winery\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x16\n" +
	"\x06region\x18\x02 \x01(\tR\x06region\x12\x18\n" +
	"\acountry\x18\x03 \x01(\tR\acountry\x12\x15\n" +
	"\x03url\x18\x04 \x01(\tH\x00R\x03url\x88\x01\x01B\x06\n" +
	"\x04_url\"[\n" +
	"\tComponent\x12\x1a\n" +
	"\bvarietal\x18\x01 \x01(\tR\bvarietal\x12#\n" +
	"\n" +
	"percentage\x18\x02 \x01(\rH\x00R\n" +
	"percentage\x88\x01\x01B\r\n" +
	"\v_percentage\">\n" +
	"\x11ShowNotFoundError\x12\x19\n" +
	"\bmessage_\x18\x01 \x01(\tR\amessage\x12\x0e\n" +
	"\x02id\x18\x02 \x01(\tR\x02id\"\x1d\n" +
	"\vShowRequest\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\"\x8a\x02\n" +
	"\fShowResponse\x12\x0e\n" +
	"\x02id\x18\b \x01(\tR\x02id\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12'\n" +
	"\x06winery\x18\x03 \x01(\v2\x0f.storage.WineryR\x06winery\x12\x18\n" +
	"\avintage\x18\x04 \x01(\rR\avintage\x124\n" +
	"\vcomposition\x18\x05 \x03(\v2\x12.storage.ComponentR\vcomposition\x12%\n" +
	"\vdescription\x18\x06 \x01(\tH\x00R\vdescription\x88\x01\x01\x12\x1b\n" +
	"\x06rating\x18\a \x01(\rH\x01R\x06rating\x88\x01\x01B\x0e\n" +
	"\f_descriptionB\t\n" +
	"\a_rating\"\xf8\x01\n" +
	"\n" +
	"AddRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12'\n" +
	"\x06winery\x18\x02 \x01(\v2\x0f.storage.WineryR\x06winery\x12\x18\n" +
	"\avintage\x18\x03 \x01(\rR\avintage\x124\n" +
	"\vcomposition\x18\x04 \x03(\v2\x12.storage.ComponentR\vcomposition\x12%\n" +
	"\vdescription\x18\x05 \x01(\tH\x00R\vdescription\x88\x01\x01\x12\x1b\n" +
	"\x06rating\x18\x06 \x01(\rH\x01R\x06rating\x88\x01\x01B\x0e\n" +
	"\f_descriptionB\t\n" +
	"\a_rating\"#\n" +
	"\vAddResponse\x12\x14\n" +
	"\x05field\x18\x01 \x01(\tR\x05field\"\x1f\n" +
	"\rRemoveRequest\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\"\x10\n" +
	"\x0eRemoveResponse\"\x96\x01\n" +
	"\vRateRequest\x125\n" +
	"\x05field\x18\x01 \x03(\v2\x1f.storage.RateRequest.FieldEntryR\x05field\x1aP\n" +
	"\n" +
	"FieldEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\rR\x03key\x12,\n" +
	"\x05value\x18\x02 \x01(\v2\x16.storage.ArrayOfStringR\x05value:\x028\x01\"%\n" +
	"\rArrayOfString\x12\x14\n" +
	"\x05field\x18\x01 \x03(\tR\x05field\"\x0e\n" +
	"\fRateResponse\"8\n" +
	"\x0fMultiAddRequest\x12%\n" +
	"\x05field\x18\x01 \x03(\v2\x0f.storage.BottleR\x05field\"\xf4\x01\n" +
	"\x06Bottle\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12'\n" +
	"\x06winery\x18\x02 \x01(\v2\x0f.storage.WineryR\x06winery\x12\x18\n" +
	"\avintage\x18\x03 \x01(\rR\avintage\x124\n" +
	"\vcomposition\x18\x04 \x03(\v2\x12.storage.ComponentR\vcomposition\x12%\n" +
	"\vdescription\x18\x05 \x01(\tH\x00R\vdescription\x88\x01\x01\x12\x1b\n" +
	"\x06rating\x18\x06 \x01(\rH\x01R\x06rating\x88\x01\x01B\x0e\n" +
	"\f_descriptionB\t\n" +
	"\a_rating\"(\n" +
	"\x10MultiAddResponse\x12\x14\n" +
	"\x05field\x18\x01 \x03(\tR\x05field\"Q\n" +
	"\x12MultiUpdateRequest\x12\x10\n" +
	"\x03ids\x18\x01 \x03(\tR\x03ids\x12)\n" +
	"\abottles\x18\x02 \x03(\v2\x0f.storage.BottleR\abottles\"\x15\n" +
	"\x13MultiUpdateResponse2\xaa\x03\n" +
	"\aStorage\x12=\n" +
	"\x04List\x12\x14.storage.ListRequest\x1a\x1f.storage.StoredBottleCollection\x123\n" +
	"\x04Show\x12\x14.storage.ShowRequest\x1a\x15.storage.ShowResponse\x120\n" +
	"\x03Add\x12\x13.storage.AddRequest\x1a\x14.storage.AddResponse\x129\n" +
	"\x06Remove\x12\x16.storage.RemoveRequest\x1a\x17.storage.RemoveResponse\x123\n" +
	"\x04Rate\x12\x14.storage.RateRequest\x1a\x15.storage.RateResponse\x12?\n" +
	"\bMultiAdd\x12\x18.storage.MultiAddRequest\x1a\x19.storage.MultiAddResponse\x12H\n" +
	"\vMultiUpdate\x12\x1b.storage.MultiUpdateRequest\x1a\x1c.storage.MultiUpdateResponseB\fZ\n" +
	"/storagepbb\x06proto3"

var (
	file_goagen_cellar_storage_proto_rawDescOnce sync.Once
	file_goagen_cellar_storage_proto_rawDescData []byte
)

func file_goagen_cellar_storage_proto_rawDescGZIP() []byte {
	file_goagen_cellar_storage_proto_rawDescOnce.Do(func() {
		file_goagen_cellar_storage_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_goagen_cellar_storage_proto_rawDesc), len(file_goagen_cellar_storage_proto_rawDesc)))
	})
	return file_goagen_cellar_storage_proto_rawDescData
}

var file_goagen_cellar_storage_proto_msgTypes = make([]protoimpl.MessageInfo, 21)
var file_goagen_cellar_storage_proto_goTypes = []any{
	(*ListRequest)(nil),            // 0: storage.ListRequest
	(*StoredBottleCollection)(nil), // 1: storage.StoredBottleCollection
	(*StoredBottle)(nil),           // 2: storage.StoredBottle
	(*Winery)(nil),                 // 3: storage.Winery
	(*Component)(nil),              // 4: storage.Component
	(*ShowNotFoundError)(nil),      // 5: storage.ShowNotFoundError
	(*ShowRequest)(nil),            // 6: storage.ShowRequest
	(*ShowResponse)(nil),           // 7: storage.ShowResponse
	(*AddRequest)(nil),             // 8: storage.AddRequest
	(*AddResponse)(nil),            // 9: storage.AddResponse
	(*RemoveRequest)(nil),          // 10: storage.RemoveRequest
	(*RemoveResponse)(nil),         // 11: storage.RemoveResponse
	(*RateRequest)(nil),            // 12: storage.RateRequest
	(*ArrayOfString)(nil),          // 13: storage.ArrayOfString
	(*RateResponse)(nil),           // 14: storage.RateResponse
	(*MultiAddRequest)(nil),        // 15: storage.MultiAddRequest
	(*Bottle)(nil),                 // 16: storage.Bottle
	(*MultiAddResponse)(nil),       // 17: storage.MultiAddResponse
	(*MultiUpdateRequest)(nil),     // 18: storage.MultiUpdateRequest
	(*MultiUpdateResponse)(nil),    // 19: storage.MultiUpdateResponse
	nil,                            // 20: storage.RateRequest.FieldEntry
}
var file_goagen_cellar_storage_proto_depIdxs = []int32{
	2,  // 0: storage.StoredBottleCollection.field:type_name -> storage.StoredBottle
	3,  // 1: storage.StoredBottle.winery:type_name -> storage.Winery
	4,  // 2: storage.StoredBottle.composition:type_name -> storage.Component
	3,  // 3: storage.ShowResponse.winery:type_name -> storage.Winery
	4,  // 4: storage.ShowResponse.composition:type_name -> storage.Component
	3,  // 5: storage.AddRequest.winery:type_name -> storage.Winery
	4,  // 6: storage.AddRequest.composition:type_name -> storage.Component
	20, // 7: storage.RateRequest.field:type_name -> storage.RateRequest.FieldEntry
	16, // 8: storage.MultiAddRequest.field:type_name -> storage.Bottle
	3,  // 9: storage.Bottle.winery:type_name -> storage.Winery
	4,  // 10: storage.Bottle.composition:type_name -> storage.Component
	16, // 11: storage.MultiUpdateRequest.bottles:type_name -> storage.Bottle
	13, // 12: storage.RateRequest.FieldEntry.value:type_name -> storage.ArrayOfString
	0,  // 13: storage.Storage.List:input_type -> storage.ListRequest
	6,  // 14: storage.Storage.Show:input_type -> storage.ShowRequest
	8,  // 15: storage.Storage.Add:input_type -> storage.AddRequest
	10, // 16: storage.Storage.Remove:input_type -> storage.RemoveRequest
	12, // 17: storage.Storage.Rate:input_type -> storage.RateRequest
	15, // 18: storage.Storage.MultiAdd:input_type -> storage.MultiAddRequest
	18, // 19: storage.Storage.MultiUpdate:input_type -> storage.MultiUpdateRequest
	1,  // 20: storage.Storage.List:output_type -> storage.StoredBottleCollection
	7,  // 21: storage.Storage.Show:output_type -> storage.ShowResponse
	9,  // 22: storage.Storage.Add:output_type -> storage.AddResponse
	11, // 23: storage.Storage.Remove:output_type -> storage.RemoveResponse
	14, // 24: storage.Storage.Rate:output_type -> storage.RateResponse
	17, // 25: storage.Storage.MultiAdd:output_type -> storage.MultiAddResponse
	19, // 26: storage.Storage.MultiUpdate:output_type -> storage.MultiUpdateResponse
	20, // [20:27] is the sub-list for method output_type
	13, // [13:20] is the sub-list for method input_type
	13, // [13:13] is the sub-list for extension type_name
	13, // [13:13] is the sub-list for extension extendee
	0,  // [0:13] is the sub-list for field type_name
}

func init() { file_goagen_cellar_storage_proto_init() }
func file_goagen_cellar_storage_proto_init() {
	if File_goagen_cellar_storage_proto != nil {
		return
	}
	file_goagen_cellar_storage_proto_msgTypes[2].OneofWrappers = []any{}
	file_goagen_cellar_storage_proto_msgTypes[3].OneofWrappers = []any{}
	file_goagen_cellar_storage_proto_msgTypes[4].OneofWrappers = []any{}
	file_goagen_cellar_storage_proto_msgTypes[7].OneofWrappers = []any{}
	file_goagen_cellar_storage_proto_msgTypes[8].OneofWrappers = []any{}
	file_goagen_cellar_storage_proto_msgTypes[16].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_goagen_cellar_storage_proto_rawDesc), len(file_goagen_cellar_storage_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   21,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_goagen_cellar_storage_proto_goTypes,
		DependencyIndexes: file_goagen_cellar_storage_proto_depIdxs,
		MessageInfos:      file_goagen_cellar_storage_proto_msgTypes,
	}.Build()
	File_goagen_cellar_storage_proto = out.File
	file_goagen_cellar_storage_proto_goTypes = nil
	file_goagen_cellar_storage_proto_depIdxs = nil
}
