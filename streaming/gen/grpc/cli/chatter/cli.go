...
Example:
    %[1]s chatter echoer --token "Ullam quia neque quis qui quia optio."
`, os.Args[0])
}
...
Example:
    %[1]s chatter listener --token "Quibusdam voluptate nulla autem quisquam dolorum."
`, os.Args[0])
}
...
Example:
    %[1]s chatter summary --token "Dignissimos sed perspiciatis similique."
`, os.Args[0])
}
...
Example:
    %[1]s chatter subscribe --token "Consequatur facilis perferendis ad quos."
`, os.Args[0])
}
...
Example:
    %[1]s chatter history --view "Ex dolorem qui rerum." --token "Dolor nihil autem ut temporibus."
`, os.Args[0])
}
