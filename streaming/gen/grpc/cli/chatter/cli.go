...
Example:
    %[1]s chatter echoer --token "Maxime enim exercitationem non."
`, os.Args[0])
}
...
Example:
    %[1]s chatter listener --token "Laudantium cupiditate aut tempore quae."
`, os.Args[0])
}
...
Example:
    %[1]s chatter summary --token "Non fugit et id cum cum quisquam."
`, os.Args[0])
}
...
Example:
    %[1]s chatter subscribe --token "Cum et deserunt quo molestiae officia ex."
`, os.Args[0])
}
...
Example:
    %[1]s chatter history --view "Aliquid in." --token "Mollitia debitis unde vero."
`, os.Args[0])
}
