...
import (
	...
	"goa.design/clue/log"
	retryapi "goa.design/examples/retry"
...
func main() {
	...
	{
		retrySvc = retryapi.NewRetry()
	}

	// Wrap the services in endpoints that can be invoked from other services
	// potentially running in different processes.
	var (
		retryEndpoints *retry.Endpoints
	)
	{
		retryEndpoints = retry.NewEndpoints(retrySvc)
		retryEndpoints.Use(debug.LogPayloads())
		retryEndpoints.Use(log.Endpoint)
	}
	...
	case "development":
		{
			...
			handleHTTPServer(ctx, u, retryEndpoints, &wg, errc, *dbgF)
		}

		{
			addr := "grpc://localhost:18081"
			u, err := url.Parse(addr)
			if err != nil {
				log.Fatalf(ctx, err, "invalid URL %#v\n", addr)
			}
			...
			handleGRPCServer(ctx, u, retryEndpoints, &wg, errc, *dbgF)
		}
