...
import (
	...
	"goa.design/clue/log"
	retrysvr "goa.design/examples/retry/gen/http/retry/server"
...
func handleHTTPServer(ctx context.Context, u *url.URL, retryEndpoints *retry.Endpoints, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	{
		eh := errorHandler(ctx)
		retryServer = retrysvr.New(retryEndpoints, mux, dec, enc, eh, nil)
	}

	// Configure the mux.
	retrysvr.Mount(mux, retryServer)
	...
	for _, m := range retryServer.Mounts {
