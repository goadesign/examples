...
import (
	...
	"goa.design/clue/log"
	interceptorsapi "goa.design/examples/interceptors"
...
func main() {
	...
	var (
		interceptorsSvc interceptors.Service
	)
	{
	...
	{
		interceptorsInterceptors = interceptorsex.NewInterceptorsServerInterceptors()
	}

	// Wrap the services in endpoints that can be invoked from other services
	// potentially running in different processes.
	var (
		interceptorsEndpoints *interceptors.Endpoints
	)
	{
		interceptorsEndpoints = interceptors.NewEndpoints(interceptorsSvc, interceptorsInterceptors)
		interceptorsEndpoints.Use(debug.LogPayloads())
		interceptorsEndpoints.Use(log.Endpoint)
	}
	...
	case "localhost":
		{
			...
			handleHTTPServer(ctx, u, interceptorsEndpoints, &wg, errc, *dbgF)
		}

		{
			...
			handleGRPCServer(ctx, u, interceptorsEndpoints, &wg, errc, *dbgF)
		}
